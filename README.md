...
### 7. 服务管理
- 服务信息登记（服务器关联、分类、版本、端口映射）
- 🔐 服务账户与密码管理（加密存储，支持复制）
- 端口映射表管理（内网端口→映射端口一对一，支持协议/备注）
- 关键词搜索（服务名/IP 模糊匹配）
...
│   ├── run.py                 # 应用入口
│   └── init_db.py             # 数据库初始化（Python版）
│   └── init_db.sql            # 数据库初始化（SQL版）
│
├── frontend/                   # 前端Vue应用
│   ├── src/
│   │   ├── api/               # API请求模块
...
# 方式一：Python 脚本初始化（推荐，含种子数据）
python init_db.py

# 方式二：SQL 脚本初始化
mysql -h mysql -u root -p ops_platform < init_db.sql
```
...
### v1.3.0 (2026-05)
- 🗂 端口映射表改造：新增 `service_ports` 独立表替代 `services` 表逗号分隔的 `inner_port`/`mapped_port` 字段，支持内网端口到映射端口一对一精确映射及协议/备注
- 🗑 清理旧字段：移除 `services` 表的 `inner_port`、`mapped_port` 列
- 🧹 数据库脚本合并：清理全部升级脚本（`upgrade_*.sql`/`upgrade_*.py`/`update.sql`），统一归入 `init_db.py` + `init_db.sql`
- 🎨 前端端口列统一：服务列表、服务器详情、项目详情的端口列均改为「端口映射」格式展示（`80→180 TCP`）
- 📋 导出优化：服务 Excel 导出去掉独立的「内部端口」「映射端口」列，仅保留「端口映射详情」
- 🐛 代码清理：移除前端未使用的 `parsePorts` 函数和 `Edit` 图标导入，修复 `ProjectDetail.vue` 隐式 any 类型错误
