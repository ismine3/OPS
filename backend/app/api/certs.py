...
def create_cert():
    ...
    try:
        ...
        if cert_expire_time:
            try:
                if isinstance(cert_expire_time, str):
                    expire_dt = datetime.datetime.strptime(cert_expire_time, '%Y-%m-%d')
                else:
                    expire_dt = cert_expire_time
                remaining_days = (expire_dt - datetime.datetime.now()).days
                
                # 如果有生成时间，计算有效期
                if data.get('cert_generate_time'):
                    if isinstance(data['cert_generate_time'], str):
                        generate_dt = datetime.datetime.strptime(data['cert_generate_time'], '%Y-%m-%d')
                    else:
                        generate_dt = data['cert_generate_time']
...
def update_cert(cert_id):
    ...
    try:
        ...
        if 'cert_expire_time' in data and data['cert_expire_time']:
            try:
                if isinstance(data['cert_expire_time'], str):
                    expire_dt = datetime.datetime.strptime(data['cert_expire_time'], '%Y-%m-%d')
                else:
                    expire_dt = data['cert_expire_time']
                remaining_days = (expire_dt - datetime.datetime.now()).days
    ...
    try:
        ...
        with zipfile.ZipFile(memory_file, 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            if cert['cert_file_path'] and os.path.exists(cert['cert_file_path']):
                zf.write(cert['cert_file_path'], 'cert.pem')

            # 添加私钥文件（如果存在）
            if cert['key_file_path'] and os.path.exists(cert['key_file_path']):
                zf.write(cert['key_file_path'], 'key.pem')
...
def deploy_cert(cert_id):
    """
    远程部署证书
    请求体: { "server_id": 1, "remote_path": "/etc/nginx/ssl/" }
    ...
    try:
        ...
        # 查询服务器信息
        cursor.execute("""
            SELECT inner_ip, os_user, os_password
        ...
        try:
            ssh.connect(
                ...
                username=server['os_user'],
                password=server['os_password'],
                timeout=30
            )
        except Exception as e:
            return jsonify({
