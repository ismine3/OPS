-- =============================================================================
-- OPS 数据库初始化脚本（完整 SQL 版本）
-- =============================================================================
-- 用途: 创建所有数据库表、默认数据，与 init_db.py 功能完全等价。
--       可作为 Docker 首次启动时或独立部署时的一键初始化脚本。
--
-- 用法:  mysql -u root -p < init_db.sql
--       或:  mysql -u root -p ops_platform < init_db.sql
--
...
-- =============================================================================
-- 4. 插入流水线配置可选值种子数据（51条）
--    使用 INNER JOIN 子查询获取 config_id
-- =============================================================================
...
-- =============================================================================
-- 5. 管理员账户
--    ⚠️  密码哈希需要使用 Python werkzeug generate_password_hash() 生成。
--    请在 init_db.py 中设置 ADMIN_PASSWORD 环境变量后运行，或手动执行：
--    
--    # 生成哈希（Python）:
--    python -c "from werkzeug.security import generate_password_hash; print(generate_password_hash('你的密码'))"
--    
--    然后将下方的 __ADMIN_PASSWORD_HASH__ 替换为实际的哈希值：
-- =============================================================================
-- INSERT IGNORE INTO `users` (`username`, `password_hash`, `display_name`, `role`, `is_active`)
-- VALUES ('admin', '__ADMIN_PASSWORD_HASH__', '系统管理员', 'admin', TRUE);
...
-- =============================================================================
-- 7. 修正 pipeline_configs 的 pipeline_type（v1 升级时未设）
-- =============================================================================
