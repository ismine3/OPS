 ...
 * 检测 SQL 关键字和危险字符
 */
export const isSafeInput = (value: any) => {
 ...
 */
export const sanitizeInput = (_rule: any, value: any, callback: (error?: Error) => void) => {
 ...
 * 检测危险HTML/JS模式
 */
export const isSafeHTML = (value: any) => {
  if (!value || typeof value !== 'string') return true
 ...
 */
export const sanitizeHTML = (_rule: any, value: any, callback: (error?: Error) => void) => {
  if (!value) { callback(); return }
  if (!isSafeHTML(value)) {
 ...
 */
export const isSafeText = (value: any) => {
 ...
 */
export const safeText = (_rule: any, value: any, callback: (error?: Error) => void) => {
  if (!value) { callback(); return }
  if (!isSafeInput(value)) {
    callback(new Error('输入包含不允许的SQL关键字或特殊字符'))
...
export const isValidIP = (value: any) => {
  ...
  return match.slice(1, 5).every((seg: string) => parseInt(seg, 10) >= 0 && parseInt(seg, 10) <= 255)
}

/**
 * IPv4地址验证 - Element Plus validator
 */
export const ipValidator = (_rule: any, value: any, callback: (error?: Error) => void) => {
  if (!value) { callback(); return }
  if (!isValidIP(value)) {
 ...
 */
export const isValidPort = (value: any) => {
  if (!value) return true
  const ports = value.toString().split(',')
  const portPattern = /^\d+$/
  return ports.every((p: string) => {
 ...
 */
export const portValidator = (_rule: any, value: any, callback: (error?: Error) => void) => {
  if (!value) { callback(); return }
 ...
 */
export const domainValidator = (_rule: any, value: any, callback: (error?: Error) => void) => {
  if (!value) { callback(); return }
 ...
 */
export const urlValidator = (_rule: any, value: any, callback: (error?: Error) => void) => {
  if (!value) { callback(); return }
  if (!isValidURL(value)) {
 ...
 * 5段格式：分 时 日 月 周
 */
export const isValidCron = (value: any) => {
  if (!value) return true
 ...
 */
export const cronValidator = (_rule: any, value: any, callback: (error?: Error) => void) => {
  if (!value) { callback(); return }
 ...
 * 以 / 开头
 */
export const isValidPath = (value: any) => {
  if (!value) return true
  return /^\/[^\s]*$/.test(value)
}
 ...
 */
export const pathValidator = (_rule: any, value: any, callback: (error?: Error) => void) => {
  if (!value) { callback(); return }
 ...
 * 长度限制验证器工厂函数
 */
export const maxLength = (n: number) => (_rule: any, value: any, callback: (error?: Error) => void) => {
  if (!value) { callback(); return }
  if (value.length > n) {
 ...
 */
export const passwordStrength = (_rule: any, value: any, callback: (error?: Error) => void) => {
  if (!value) { callback(); return }
  if (value.length < 6) {
 ...
 * 验证单个端口是否在指定范围内
 */
export const isValidPortRange = (value: any, min: number = 1, max: number = 65535) => {
  if (!value) return true
 ...
 * 可指定范围
 */
export const validatePortRange = (min: number = 1, max: number = 65535) => (_rule: any, value: any, callback: (error?: Error) => void) => {
  if (!value) { callback(); return }
  const port = parseInt(value, 10)
 ...
 * 允许空值，非空时做安全检查
 */
export const isSafeSearch = (value: any) => {
  if (!value) return true
 ...
 */
export const searchValidator = (_rule: any, value: any, callback: (error?: Error) => void) => {
  if (!value) { callback(); return }
  if (!isSafeInput(value)) {
